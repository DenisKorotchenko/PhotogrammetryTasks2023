...
public:
    ...
    bool operator()(const T* camera_extrinsics, // положение камеры:   [6] = {translation[3], rotation[3]} (разное для всех кадров, т.к. каждая фотография со своего ракурса)
        ...
        T rotated_points[3];
        ceres::AngleAxisRotatePoint(rotation, points, rotated_points);

        // Проецируем точку на фокальную плоскость матрицы (т.е. плоскость Z=фокальная длина)
        T projected_points[2];
        for (int i = 0; i < 2; i++) {
            projected_points[i] = rotated_points[i] / rotated_points[2];
        }
        ...
        T r = projected_points[0] * projected_points[0] + projected_points[1] * projected_points[1];
        T distortion_points[2];
        for (int i = 0; i < 2; i++) {
            distortion_points[i] = projected_points[i] + k1 * r + k2 * r * r;
        }
#endif

        // Домножаем на f, тем самым переводя в пиксели
        T f = camera_intrinsics[2];
        T distortion_pixels[2];
        for (int i = 0; i < 2; i++) {
            distortion_pixels[i] = distortion_points[i] * f;
        }
        ...
        T moved_pixels[2];
        moved_pixels[0] = distortion_pixels[0] + cx;
        moved_pixels[1] = distortion_pixels[1] + cy;

        // Теперь по спроецированным координатам не забудьте посчитать невязку репроекции
        residuals[0] = moved_pixels[0] - observed_x;
        residuals[1] = moved_pixels[1] - observed_y;
    ...
    for (size_t i = 0; i < tie_points.size(); ++i) {
        ...
        for (size_t ci = 0; ci < track.img_kpt_pairs.size(); ++ci) {
            ...
            if (ENABLE_OUTLIERS_FILTRATION_COLINEAR && ENABLE_BA) {
                ...
                phg::decomposeUndistortedPMatrix(tmp, camera_origin2, cameras[track.img_kpt_pairs[ci].second]);

                vector3d p1 = track_point - camera_origin;
                vector3d p2 = track_point - camera_origin2;
                if (p1.dot(p2) >= sqrt(p1.dot(p1)) * sqrt(p2.dot(p2)) * 0.999) {
                    should_be_disabled = true;
                }
            }
